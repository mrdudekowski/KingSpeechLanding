/**
 * KingSpeech GAS Webhook - Fixed Version
 * Обработка заявок с лендинга и отправка в Telegram + Google Sheets
 * Исправлены ошибки CORS и улучшена стабильность
 */
...
function createResponse(success, message, data) {
  ...
  // Устанавливаем заголовки CORS (исправленный способ)
  output.setHeaders({
  ...
  try {
    ...
    dates.forEach((date, index) => {
      if (new Date(date[0]) < cutoffDate) {
        rowsToDelete.push(index + 2); // +2 потому что начинаем с строки 2
      }
    });
